...
/**
 * Bulk Create positions (Admin only)
 */
exports.createPositionsBulk = async (req, res) => {
  try {
    ...
    const { positions } = req.body; // Expecting array of position objects
    ...
    for (const [index, posData] of positions.entries()) {
      const { name, seats, nominationOpens, nominationCloses, votingOpens, votingCloses } = posData;

      // Basic Validation
      if (!name || seats === undefined || !nominationOpens || !nominationCloses || !votingOpens || !votingCloses) {
        errors.push({ index, error: 'All fields are required', data: posData });
        continue;
      }

      const seatsInt = parseInt(seats);
      if (isNaN(seatsInt) || seatsInt <= 0) {
        errors.push({ index, error: 'Seats must be a positive number', data: posData });
        continue;
      }
      ...
      if (isNaN(nomOpen.getTime()) || isNaN(nomClose.getTime()) || isNaN(voteOpen.getTime()) || isNaN(voteClose.getTime())) {
        errors.push({ index, error: 'Invalid date format', data: posData });
        continue;
      }

      if (nomClose <= nomOpen) {
        errors.push({ index, error: 'Nomination close date must be after open date', data: posData });
        continue;
      }
      if (voteClose <= voteOpen) {
        errors.push({ index, error: 'Voting close date must be after open date', data: posData });
        continue;
      }
      if (voteOpen < nomClose) {
        errors.push({ index, error: 'Voting period must start after nomination period ends', data: posData });
        continue;
      }
      ...
      try {
        ...
        errors.push({ index, error: 'Database error', details: dbError.message, data: posData });
      }
    }
    ...
    if (createdPositions.length > 0) {
      await logAudit({
        ...
        entity: 'position',
        entityId: 'BULK',
        payload: { count: createdPositions.length, names: createdPositions.map(p => p.name) },
      });
    }

    res.status(201).json({
      message: `Processed ${positions.length} items. Created ${createdPositions.length} positions.`,
      createdPositions,
    ...
    res.status(500).json({ error: 'Failed to process bulk creation', details: error.message });
  }
};
